...
const PreferenceDialog = ({
  ...
  const [selectedExperienceLevels, setSelectedExpereinceLevels] = useState<
    string[]
  >([]);
  ...
  useEffect(() => {
    ...
    const fetchPresetPreferences = async () => {
      try {
        ...
        setSelectedJobTypes(response.data.preference.job_type);
        setSelectedExpereinceLevels(response.data.preference.experience_level);
      } catch (error) {
